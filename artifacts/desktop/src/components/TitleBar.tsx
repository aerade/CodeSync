...
export function TitleBar({ onOpenCommandPalette }: Props) {
  ...
  return (
    <div className="titlebar-drag flex items-center justify-between h-9 px-2 bg-[#0F0F11] border-b border-white/5 select-none">
      ...
      <div className="flex items-center gap-2 min-w-[120px]">
        {showMacButtons ? (
          <div className="titlebar-no-drag flex items-center gap-1.5 px-1.5">
            <button type="button" onClick={() => desktop().window.close()}
              className="w-3 h-3 rounded-full bg-[#FF5F57] hover:opacity-80 transition-opacity" aria-label="Закрыть" />
            <button type="button" onClick={() => desktop().window.minimize()}
              className="w-3 h-3 rounded-full bg-[#FEBC2E] hover:opacity-80 transition-opacity" aria-label="Свернуть" />
            <button type="button" onClick={() => desktop().window.maximizeToggle()}
              className="w-3 h-3 rounded-full bg-[#28C840] hover:opacity-80 transition-opacity" aria-label="Развернуть" />
          </div>
        ) : (
          <div className="flex items-center gap-2 pl-1.5">
            <div className="w-5 h-5 rounded-md bg-gradient-to-br from-[#F97316] to-[#EA580C] grid place-items-center text-[10px] font-bold text-white shadow-[0_0_12px_rgba(249,115,22,0.4)]">
              C
            </div>
      ...
      {/* right */}
      <div className="titlebar-no-drag flex items-center gap-1.5 min-w-[120px] justify-end">
        <UserMenu />
        {native && !showMacButtons && (
          <div className="flex items-center">
            <button type="button" onClick={() => desktop().window.minimize()}
              className="h-9 w-11 grid place-items-center hover:bg-white/5 text-zinc-400" aria-label="Свернуть">
              <Minus className="w-3.5 h-3.5" />
            </button>
            <button type="button" onClick={() => desktop().window.maximizeToggle()}
              className="h-9 w-11 grid place-items-center hover:bg-white/5 text-zinc-400" aria-label="Развернуть">
              {isMax ? <Square className="w-3 h-3" /> : <Maximize2 className="w-3 h-3" />}
            </button>
            <button type="button" onClick={() => desktop().window.close()}
              className="h-9 w-11 grid place-items-center hover:bg-[#E26F6F] text-zinc-400" aria-label="Закрыть">
