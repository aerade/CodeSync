                    ...
                    >
                      ...
                      {section === "editor" && (
                        <div>
                          ...
                          <SliderPicker label="Размер шрифта (px)" value={settings.fontSize} onChange={(v) => set({ fontSize: v })} min={10} max={24} step={1} presets={[12, 14, 16, 18, 20]} />
                          <SectionLabel>Отступы</SectionLabel>
                          <SliderPicker label="Размер табуляции (пробелов)" value={settings.tabSize} onChange={(v) => set({ tabSize: v })} min={1} max={8} step={1} presets={[2, 4, 8]} />
                          ...
                          <Toggle value={settings.showMinimap} onChange={(v) => set({ showMinimap: v })} label="Миникарта кода" desc="Показывать обзор кода в правой части редактора" />
                        </div>
                      )}
                      ...
                      {section === "presence" && (
                        <div>
                          ...
                          <Toggle value={settings.showMouseCursors} onChange={(v) => set({ showMouseCursors: v })} label="Курсоры мыши" desc="Видеть где находится мышь других участников" />
                          <Toggle value={settings.showFilePresence} onChange={(v) => set({ showFilePresence: v })} label="Присутствие в файлах" desc="Показывать в каком файле находится участник" />
                          <Toggle value={settings.showEditorCursors} onChange={(v) => set({ showEditorCursors: v })} label="Курсоры в редакторе" desc="Позиция текстового курсора в редакторе" />
                          <Toggle value={settings.showChatMessages} onChange={(v) => set({ showChatMessages: v })} label="Чат участников" desc="Получать сообщения чата от других участников" />
                        </div>
                      )}
                      ...
                      {section === "notifications" && (
                        <div>
                          ...
                          <Toggle value={settings.notifyOnJoin} onChange={(v) => set({ notifyOnJoin: v })} label="Подключение участников" desc="Уведомлять когда кто-то подключается к комнате" />
                          <Toggle value={settings.notifyOnMessage} onChange={(v) => set({ notifyOnMessage: v })} label="Новые сообщения" desc="Уведомлять о новых сообщениях в чате" />
                          <Toggle value={settings.showChatMessages} onChange={(v) => set({ showChatMessages: v })} label="Показывать сообщения чата" desc="Отображать сообщения от участников комнаты" />
                        </div>
                      )}
                        ...
                        <div>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                            {SHORTCUTS.map(({ keys, desc }) => (
                              <div key={desc} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: 8, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" }}>
                                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>{desc}</span>
                                <div style={{ display: "flex", gap: 3 }}>
                                  {keys.map((k, i) => (
                                    <span key={i} style={{ padding: "2px 6px", borderRadius: 5, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", fontSize: 10, fontWeight: 600, color: "rgba(255,255,255,0.7)", fontFamily: "JetBrains Mono, monospace" }}>{k}</span>
                                  ))}
                                </div>
                              </div>
                            ))}
